...
public interface NominationsRepository extends JpaRepository<Nominations,Long> {
    ...
    @Query("SELECT r.user FROM Nominations r WHERE r.skillSet.skillName=:skillName AND r.manager_approval='approved' AND r.admin_approval='approved'")
    List<User> getApprovedTrainers(@Param("skillName") String skillName);
}
