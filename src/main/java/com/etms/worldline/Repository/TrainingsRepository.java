...
public interface TrainingsRepository extends JpaRepository<Trainings,Long> {
    ...
    @Query("SELECT new com.etms.worldline.payload.response.TrainingsResponse(t.id,t.training_name,t.training_date,t.slot_from,t.slot_to,t.trainer.user.username,t.trainer.user.user_id) from Trainings t where t.slot_to<=:oneMonthago AND t.slot_to>=:currentDate ORDER BY t.slot_from ASC")
    ...
    @Query("SELECT new com.etms.worldline.payload.response.TrainingsResponse(t.id, t.training_name, t.training_date, t.slot_from, t.slot_to, t.trainer.user.user_id, t.createdAt, t.modifiedAt) " +
            "FROM Trainings t " +
            "WHERE t.training_date <= :oneMonthAgoDate " +
            "ORDER BY t.slot_from ASC")
    List<TrainingsResponse> findTrainingsOlderThanOneMonth(@Param("oneMonthAgoDate") LocalDate oneMonthAgoDate);
    void deleteById(Long id);
    @Query("SELECT new com.etms.worldline.payload.response.TrainingsResponse(t.id,t.training_name,t.training_date,t.slot_from,t.slot_to,t.trainer.user.username,t.trainer.user.user_id) from Trainings t ORDER BY t.slot_from ASC")
    List<TrainingsResponse> getAlltrainings();
