...
//                    User employee = userRepository.findByEmail(user.getEmail()).get();
                        ...
                        ManagerdetailsDTO manager=new ManagerdetailsDTO(logUser.getUsername(),logUser.getEmail(),logUser.getDas_id(),logUser.getEntity(),logUser.getGbl(),logUser.getLocation(),logUser.getManager_name(),logUser.getManager_id(),null);
                ...
                return ResponseEntity.status(401).body(new ErrorResponse(40400, "User is not found"));
            }
        }
        catch (Exception e){
            e.printStackTrace();
...
//    }
    ...
    public ResponseEntity<?> saveUserCertifications(UserCertRequest userCertRequest,MultipartFile file) throws IOException {
        try {
            ...
            // Save the profile picture
            String uploadDir = "C:\\Users\\w136637\\OneDrive - Worldline SA\\Bureau\\Backend-Main\\wgs_internalapp\\user-certificates\\";
            String certPath = uploadDir + file.getOriginalFilename();
            File profile = new File(certPath);
            file.transferTo(profile);
            UserCertifications userCertifications = new UserCertifications(user, userCertRequest.getCert_name(),userCertRequest.getCert_url(), file.getOriginalFilename());
    ...
    public ResponseEntity<?> passwordReset(Long user_id,@RequestBody PassowordResetRequest passowordResetRequest){
        ...
        if(passowordResetRequest.getNew_password().equals(passowordResetRequest.getConfirm_password())){
            ...
            return ResponseEntity.badRequest().body(new MessageResponse(40002,"Password does not match"));
        }
        }
        else{
          return ResponseEntity.badRequest().body(new MessageResponse(40002,"Password is not entered correctly"));
        }
           }
